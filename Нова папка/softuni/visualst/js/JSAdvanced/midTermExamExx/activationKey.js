function genActivationKey(input) {
    ///input & result data
    let activationKey = input.shift()
    let instructions = input.slice()
    ///loop throuht instructions
        ...
        switch (instructionName) {
            case `Contains`: {
                ...
                if (activationKey.includes(substring)) {
                    ...
                    console.log(`Substing not found!`);
                }
                break;
            }
            case "Flip": {
   ...
   /// print result (Generated activation key)
    console.log(`Your activation key is : ${activationKey}`);
} genActivationKey(["abcdefghijklmnopqrstuvwxyz",
    "Slice>>>2>>>6",
    "Flip>>>Upper>>>3>>>14",
    "Flip>>>Lower>>>5>>>7",
    "Contains>>>def",
